...
struct Params
{
    ...
    int matingnum; // number of matings for each queen
...
{
int IDtag;
bool female; // true for female, false for male
// genome
...
void StartFromLast (istream & in, Params & Par, Population & Pop)
{
	...
	for (unsigned int i = 0; i < Pop.size(); i++)
		{
        for (int mate = 0; mate <Par.matingnum; mate ++)
        {
		    for (int task = 0; task<Par.tasks; task++) 
                {
                in >> tmp;
                Pop[i].males[mate].threshold[task] = tmp;
                }
        }
		for (int task=0; task<Par.tasks; task++)
			{
...
void InitFounders(Population &Pop, Params &Par)
	...
	for (unsigned int i = 0; i < Pop.size(); i++)
		{
        Pop[i].males.resize(Par.matingnum);
		Pop[i].queen.threshold.resize(Par.tasks);
        for (unsigned int ind=0; ind <Pop[i].males.size(); ind ++)
                {
		        Pop[i].males[ind].threshold.resize(Par.tasks);
		        for(int task=0; task<Par.tasks; task++)
                    Pop[i].males[ind].threshold[task]= Par.meanT[task];
                }
		for(int task=0; task<Par.tasks; task++)
		    {
...
void InitAnts(Ant & myAnt, Params & Par, Colony & myCol)
  ...
  if (Par.maxgen > 1) Inherit(myAnt, myCol.queen, myCol.males[RandomNumber(Par.matingnum)], Par);
  else 
    {
    for (int task=0; task<Par.tasks; task++)
...
//======================================================================================
void ShowColonyGenes(Colony & anyCol, Params & Par)
{
for (int mate = 0; mate < Par.matingnum; mate++)
    {
    cout << "Male " << mate << endl; 
    for (int task = 0; task < Par.tasks; task++)
        {
        cout << anyCol.males[mate].threshold[task] << " ; " ;	
        }
    cout << endl;
    }
cout << "Queen "<< endl;
for (int task = 0; task < Par.tasks; task++)
    {
        cout << anyCol.queen.threshold[task] << " ; " << endl;	
    }
...
for (unsigned int ant = 0; ant < anyCol.MyAnts.size(); ant++)
    ...
    for (int task = 0; task < Par.tasks; task++)
        {
            cout << anyCol.MyAnts[ant].threshold[task] << " ; " << endl;	
        }
    }
    cout << endl;
}
...
{
 mySexuals.resize(100 * Par.Col); // number of sexuals needed
 ...
 for (unsigned int ind = 0; ind < mySexuals.size(); ind ++)
	...
	parentCol[ind] = drawParent(Pop.size(), Pop);
      ...
      //	male or female with 50% chance
        if (Uniform() >= 0.5) mySexuals[ind].female = true;
        else mySexuals[ind].female = false; 

	Inherit(mySexuals[ind], Pop[parentCol[ind]].queen, Pop[parentCol[ind]].males[RandomNumber(Par.matingnum)], Par);
...
for (unsigned int col = 0; col < Pop.size(); col++)
	...
	do
        ...
        } while (mySexuals[mother].female==false || mySexuals[mother].mated==true); // choose a virgin female
    
	mySexuals[mother].mated = true;

	Pop[col].queen = mySexuals[mother];  // copy new females
	...
	for (int mate = 0; mate < Par.matingnum; mate++)
        {
            do
                {
                fathers[mate]  = RandomNumber(mySexuals.size()) ;
               // cout << "trying... " << fathers[mate] << endl;
                }
            while  
                (mother == fathers[mate] || mySexuals[fathers[mate]].mated == true || mySexuals[fathers[mate]].female == true) ;

            mySexuals[fathers[mate]].mated = true;
            Pop[col].males[mate] = mySexuals[fathers[mate]];
        }
    /*
    cout << "Queen " << Pop[col].queen.IDtag << endl;  
    cout << "Males " ;
	for (int mate = 0; mate < Par.matingnum; mate++)
        {
    //    mySexuals[fathers[mate]].mated = false;
        cout << Pop[col].males[mate].IDtag << " ; " ; 
        }
    cout << endl; */
    } // end for Colonies
...
if (generation - simstart_generation == Par.maxgen -1) 
	{
		...
		for (unsigned int col = 0; col < Pop.size(); col++)
	        ...
	        for (int mate = 0; mate < Par.matingnum; mate++)
                ...
                for (int task = 0; task < Par.tasks; task++)
                    {
                        lastgen << Pop[col].males[mate].threshold[task] << endl;	
                    }
                }
			for (int task = 0; task < Par.tasks; task++)
		 		{
...
//==================================================================================================================
void Write_Thresholds(ofstream & data_thresh, Params & Par, int gen)
{
    for (unsigned int it = 0; it < mySexuals.size(); it ++)
    {
	data_thresh << gen << ";";
	for (int task=0; task<Par.tasks; task++)
		{
			if (task < Par.tasks-1)
			    data_thresh << mySexuals[it].threshold[task] << ";"; 
			else 
			    data_thresh << mySexuals[it].threshold[task] << endl; 
		}
    }
}
//==================================================================================================================
void Write_Spec(ofstream & data_f, Params & Par, Population & Pop, int gen, int colony)
{
...
void StopIfSpec(Population & Pop, int generation, Params & Par, ofstream & file1, ofstream &file2, ofstream & file3, ofstream &file4, int eq_steps, string filename)
	...
	if (Check_Spec(Pop))
		...
		for (unsigned int col = 0; col < Pop.size(); col++)
			...
			Write_Spec(file3, Par, Pop, generation, col);
			
			}
        Write_Thresholds(file2, Par, generation);
...
int main(int argc, char* argv[])
{
	...
	for (int g= simstart_generation; g < simstart_generation + myPars.maxgen; g++)
	    ...
	    //ShowColonyGenes(MyColonies[0], myPars);	
	//	mygetch();
		//
		    ...
		    if (k == myPars.maxtime-1)
			 ...
			 if ((g<=100 || g%100==0) && myPars.Col>1)
			    ...
			    for (unsigned int col = 0; col < MyColonies.size(); col++)
				{
					...
					Write_Spec(out_f, myPars, MyColonies, g, col);
					 
				} // end of for colonies
                Write_Thresholds(out2, myPars, g);
